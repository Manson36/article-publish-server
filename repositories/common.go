...
type CommonRepository interface {
	...
	Update(entity, query interface{}, doc map[string]interface{}, args ...interface{}) error
...
func (c commonRepository) Remove(entity, query interface{}, args ...interface{}) error {
	return c.db.Model(entity).Where(query, args...).Updates(map[string]interface{}{
		"removed":    true,
		"removed_at": gorm.Expr("now()"), //为什么使用这个？？？
	}).Error
}

func (c commonRepository) RemoveWithTx(tx *gorm.DB, entity, query interface{}, args ...interface{}) error {
	return tx.Model(entity).Where(query, args...).Updates(map[string]interface{}{
		"removed":    true,
		"removed_at": gorm.Expr("now()"),
	}).Error
}

func (c commonRepository) Update(entity, query interface{}, doc map[string]interface{}, args ...interface{}) error {
	//return c.db.Table(c.db.NewScope(entity).TableName()).Where(query, args...).Updates(doc).Error
	return c.db.Model(entity).Where(query, args...).Updates(doc).Error
}

func (c commonRepository) UpdateWithTx(tx *gorm.DB, entity, query interface{}, doc map[string]interface{}, args ...interface{}) error {
	//return tx.Table(tx.NewScope(entity).TableName()).Where(query, args...).Updates(doc).Error
	return c.db.Model(entity).Where(query, args...).Updates(doc).Error
}
